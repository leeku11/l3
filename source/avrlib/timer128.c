...
void timer0PWMOn(void)
{
	// turn on OC0 PWM output
...
{
	// turn off OC0 PWM output
	// set OC0 (OutputCompare action) to none
	cbi(TCCR0,COM01);
	cbi(TCCR0,COM00);
}

void timer0PWMSet(u16 pwmDuty)
{
	...
	outb(OCR0, (pwmDuty&0x00FF));	// set the low 8bits of OCR1A
}
